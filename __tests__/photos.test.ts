...
describe("Lightbox functionality", () => {
  ...
  let showImage: (index: number) => void;
  let openLightbox: (image: HTMLImageElement) => void;
  let closeLightbox: () => void;
  let handleKeyDown: (e: KeyboardEvent) => void;
  let preloadAdjacentImages: ReturnType<typeof mock<(index: number) => void>>;
  ...
  beforeEach(() => {
    ...
    preloadAdjacentImages = mock((_index: number) => {
      ...
      lightboxImg.onerror = function () {
        ...
        if (announcement) {
          announcement.textContent =
            "Error loading image. Please try closing and reopening the lightbox.";
        }
      };
    ...
    photosContainer.addEventListener("click", (e) => {
      ...
      if (
        target.tagName === "IMG" &&
        target.closest("[data-photos-container]")
      ) {
  ...
  test("image onerror handler sets fallback alt text", () => {
    ...
    lightboxImg.onerror!(new Event("error"));
